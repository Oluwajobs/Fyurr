...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://mobol@localhost:5432/Fyyur' # Connecting to database
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False  # allowing for modifications
# ----------------------------------------------------------------------------#
# Models.
#----------------------------------------------------------------------------#

# defining a relationship table between artistid and venueid
...
class Venue(db.Model):
    ...
    # put genre, website link, looking for talent, and seeking description columns
    # TODO: implement any missing fields, as a database migration using Flask-Migrate
    ## Done: Added "genres, website_link, looking_for_talent, and seeking_description"
...
# TODO: implement any missing fields, as a database migration using Flask-Migrate
## Done added "website_link, looking_for_venue, seeking_description"


# TODO Implement Show and Artist models, and complete all model relationships and properties, as a database migration.
# Done. used the shows table to join the artist and venue tables.

# class Show(db.Model):
#   """Child table to arist and venue tables"""
#   __tablename__ = 'Show'
...
def search_venues():
  ...
  for result in search_result:
    data.append({
      "id": result.id, 
      "name": result.name, 
      "num_upcoming_shows": len(db.session.query(shows).filter(shows.venue_id == result.id).fliter(shows.start_time > datetime.now()).all()),
    })
  ...
  #   }]
  # }
  return render_template('pages/search_venues.html', results=response, search_term=request.form.get('search_term', ''))  # or search_term=search_term
...
def delete_venue(venue_id):
  ...
  if not error:
    flash(f"Venue {venue_id} was succesfully deleted.")
...
def search_artists():
  ...
  for result in search_result:
    data.append({
      "id": result.id,
      "name": result.name,
      "num_upcoming_show": len(db.seesion.query(shows).filter(shows.artist_id==result.id).filter(shows.start_time > datetime.now()).all()),
    })
...
def show_artist(artist_id):
  ...
  upcoming_shows_query = db.session.query(shows).join(Venue).filter(artist_id=artist_id).filter(shows.start_time > datetime.now()).all()
...
def edit_venue_submission(venue_id):
  ...
  try:
    ...
    db.session.close()
  if error:
    flash(f"An error occured. Venue {venue.name} could not be changed.")
  if not error:
    flash(f"Venue {venue.name} was successfully changed")
  return redirect(url_for('show_venue', venue_id=venue_id))

#  Create Artist
  ### Stopped Here 01/04/2022
#  ----------------------------------------------------------------
...
def shows():
  ...
  shows_query = db.session.query(shows).join(Artist).join(Venue).all()
...
def create_show_submission():
  ...
  try:
    ...
    print(request.form)  # to see if it all worked out
